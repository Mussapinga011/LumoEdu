...
const AdminUsersPage = () => {
  ...
  return (
    <div className="space-y-8 pb-20 animate-in fade-in duration-700">
      ...
      {/* Hero Header */}
      <div className="relative overflow-hidden bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-950 p-6 md:p-8 rounded-[2rem] text-white shadow-xl">
        <div className="absolute top-0 right-0 w-48 h-48 bg-blue-400/20 rounded-full blur-[80px] -translate-y-1/2 translate-x-1/2"></div>
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="text-center md:text-left">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-white/10 backdrop-blur-md rounded-full text-blue-300 text-[10px] font-black uppercase tracking-[0.15em] mb-3 border border-white/10 italic">
              <Shield size={12} /> Controle de Acessos
            </div>
            ...
            <p className="text-blue-100/70 font-medium max-w-md text-sm italic leading-tight">
              Monitore o progresso, ajuste permissões e gerencie assinaturas da sua base de alunos.
            </p>
          </div>
          
          <div className="flex flex-col gap-4 w-full md:w-auto">
            <div className="bg-white/5 backdrop-blur-md rounded-2xl p-4 border border-white/10 flex items-center justify-between gap-8">
              <div className="flex flex-col">
                <div className="text-3xl font-black text-white italic leading-none">{stats.total}</div>
                <div className="text-[9px] font-black uppercase tracking-widest text-blue-300 mt-1.5">Total de Alunos</div>
              </div>
              <div className="w-px h-10 bg-white/10"></div>
              <div className="flex flex-col text-right">
                <div className="text-3xl font-black text-amber-400 italic leading-none">{stats.premium}</div>
                <div className="text-[9px] font-black uppercase tracking-widest text-amber-300 mt-1.5 italic flex items-center gap-1 justify-end">
            ...
            <div className="flex items-center justify-center md:justify-end gap-8 text-blue-200/50">
              <div className="text-center">
                <div className="text-xl font-black text-white italic">{stats.admins}</div>
                <div className="text-[9px] uppercase font-bold tracking-[0.15em] mt-0.5 italic">Moderadores</div>
              </div>
              <div className="w-px h-5 bg-white/10"></div>
              <div className="text-center">
                <div className="text-xl font-black text-emerald-400 italic">{stats.activeToday}</div>
                <div className="text-[9px] uppercase font-bold tracking-[0.15em] mt-0.5 italic">Ativos Hoje</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-col xl:flex-row gap-4 bg-white p-4 rounded-[1.5rem] shadow-lg border border-gray-50 items-center">
         <div className="flex-1 relative w-full group">
            ...
            <input 
              ...
              className="w-full pl-12 pr-4 py-3 text-sm bg-gray-50 border-2 border-transparent focus:border-blue-500 focus:bg-white outline-none rounded-xl font-bold text-gray-800 transition-all shadow-inner italic"
            />
         </div>
         ...
         <div className="flex bg-gray-100 p-1 rounded-xl w-full xl:w-auto">
            {[
               ...
               <button 
                 ...
                 className={clsx(
                   "flex-1 xl:flex-none flex items-center justify-center gap-1.5 px-4 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all", 
                   activeTab === tab.id 
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50/50 border-b border-gray-100">
                <th className="px-3 py-2 font-black text-[9px] text-gray-400 uppercase tracking-widest italic">Estudante</th>
                <th className="px-3 py-2 font-black text-[9px] text-gray-400 uppercase tracking-widest italic">Acesso / Plano</th>
                <th className="px-3 py-2 font-black text-[9px] text-gray-400 uppercase tracking-widest italic text-center">Atividade</th>
                <th className="px-3 py-2 text-right font-black text-[9px] text-gray-400 uppercase tracking-widest italic">Ações</th>
              </tr>
            </thead>
              ...
              {filteredUsers.length === 0 ? (
                <tr>
                  <td colSpan={4} className="p-12 text-center">
                    ...
                    <h3 className="text-lg font-bold text-gray-800 uppercase italic tracking-tighter">Nenhum resultado</h3>
                    <p className="text-sm text-gray-400 font-medium italic mt-1">Refine seus filtros ou termo de busca.</p>
                  </td>
                </tr>
              ) : filteredUsers.map((u) => {
                  ...
                  <tr key={userId} className="hover:bg-blue-50/30 transition-all group">
                    <td className="px-3 py-2">
                      <div className="flex items-center gap-3">
                        <div className={clsx(
                          "w-9 h-9 rounded-lg flex items-center justify-center text-xs font-black shadow-sm ring-2 ring-white transition-transform group-hover:scale-105",
                          isAdmin ? "bg-indigo-600 text-white" : u.isPremium ? "bg-amber-400 text-white" : "bg-blue-100 text-blue-600"
                        )}>
                          {u.photoURL ? (
                             <img src={u.photoURL} alt={u.displayName} className="w-full h-full rounded-lg object-cover" />
                          ) : initials}
                        </div>
                        <div className="min-w-0">
                          <div className="font-black text-gray-800 text-sm leading-none uppercase tracking-tighter mb-1 italic flex items-center gap-1.5 truncate">
                            {u.displayName}
                            {u.isPremium && <Crown size={12} className="text-amber-500 shrink-0" fill="currentColor" />}
                          </div>
                          <div className="flex items-center gap-1.5 text-[9px] font-bold text-gray-400 tracking-wider italic truncate">
                            <span className="flex items-center gap-0.5"><Mail size={9} /> {u.email}</span>
                          </div>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-3 py-2">
                       <div className="flex items-center gap-1.5">
                          ...
                          <button 
                             ...
                             className={clsx(
                                "px-2 py-0.5 rounded-md text-[8px] font-black uppercase tracking-widest border transition-all flex items-center gap-0.5",
                    ...
                    <td className="px-3 py-2">
                       <div className="flex flex-col items-center gap-0 italic">
                          ...
                          <span className="text-[7px] font-black text-gray-300 uppercase tracking-tighter">Hoje</span>
                       </div>
                    </td>
                    ...
                    <td className="px-3 py-2 text-right">
                      <div className="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-all">
                        <button 
                          ...
                          className="p-1.5 bg-gray-50 text-gray-400 rounded-lg hover:bg-blue-600 hover:text-white transition-all text-xs"
                          title="Ficha"
                        >
                          <Edit size={14} />
                        </button>
                        <button 
                          ...
                          className="p-1.5 bg-red-50 text-red-400 rounded-lg hover:bg-red-600 hover:text-white transition-all text-xs"
                          title="Banir"
                        >
                          <Trash2 size={14} />
                        </button>
